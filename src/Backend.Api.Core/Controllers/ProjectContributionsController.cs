...
[ApiController]
public class ProjectContributionsController(ProjectContributionService service) : ControllerBase
{
    ...
    {
        var response = await _service.GetPagedAsync<ProjectContributionListResponseDto>(
    ...
    {
        var response = await _service.GetByIdAsync<ProjectContributionResponseDto>(id, ct);
        return response.ToActionResult();
    }
