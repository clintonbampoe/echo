...
public partial class MemberMapper : IMemberMapper
{
    ...
    public void Patch(MemberUpdateDto dto, Member entity)
    {
        ...
        if (dto.OtherNames != null) entity.OtherNames = dto.OtherNames;
        ...
        if (dto.JoinedDate.HasValue) entity.JoinedDate = dto.JoinedDate.Value;
        if (dto.Gender.HasValue) entity.Gender = dto.Gender.Value;
        if (dto.ResidentialAddress != null) entity.ResidentialAddress = dto.ResidentialAddress;
        if (dto.City != null) entity.City = dto.City;
        if (dto.Hometown != null) entity.Hometown = dto.Hometown;
        if (dto.Region.HasValue) entity.Region = dto.Region.Value;
        if (dto.GpsAddress != null) entity.GpsAddress = dto.GpsAddress;
        if (dto.MaritalStatus.HasValue) entity.MaritalStatus = dto.MaritalStatus.Value;
        if (dto.NextOfKin != null) entity.NextOfKin = dto.NextOfKin;
        if (dto.EmergencyContactName != null) entity.EmergencyContactName = dto.EmergencyContactName;
        if (dto.EmergencyContactPhoneNumber != null)
            entity.EmergencyContactPhoneNumber = dto.EmergencyContactPhoneNumber;
        if (dto.MemberActivityStatus.HasValue) entity.MemberActivityStatus = dto.MemberActivityStatus.Value;
    }
}
