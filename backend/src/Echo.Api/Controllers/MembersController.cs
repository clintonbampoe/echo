...
public class MembersController(MemberService service) : ApiBaseController
{
    [HttpGet]
    public async Task<IActionResult> List([FromQuery] MemberFilters filters, [FromQuery] PaginationRequest pagination, CancellationToken ct)
    {
    ...
    [HttpPost]
    public async Task<IActionResult> Create([FromBody] MemberCreateDto dto, CancellationToken ct)
    {
    ...
    [HttpPut("{id:guid}")]
    public async Task<IActionResult> Update(Guid id, [FromBody] MemberUpdateDto dto, CancellationToken ct)
    {
