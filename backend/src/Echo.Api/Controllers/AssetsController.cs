...
public class AssetsController(AssetService service) : ApiBaseController
{
    [HttpGet]
    public async Task<IActionResult> List([FromQuery] AssetFilters filters, [FromQuery] PaginationRequest pagination, CancellationToken ct)
    {
    ...
    [HttpPost]
    public async Task<IActionResult> Create([FromBody] AssetCreateDto dto, CancellationToken ct)
    {
    ...
    [HttpPut("{id:guid}")]
    public async Task<IActionResult> Update(Guid id, [FromBody] AssetUpdateDto dto, CancellationToken ct)
    {
